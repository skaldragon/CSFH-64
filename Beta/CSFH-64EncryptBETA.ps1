# NOTE THIS VERSION IS IN BETA AND IS NOT FULL PRODUCTION
function CSFH-64Encrypt{
param(
[Parameter(Mandatory=$true)][string]$Filepath,
[Parameter(Mandatory=$false)][ValidateSet("Simple","Fracture","Complex")][string]$DataMethod,
[Parameter(Mandatory=$false)][ValidateScript({$_ -gt 0})][int]$EncryptionTimes,
...
$bytes=Get-Content -Path $outPath -Encoding Byte
$byteslength=$bytes.Count - 1
$bytescount=$bytes[$byteslength..0]
[System.IO.File]::WriteAllBytes($outPath, $bytescount)
} #End IF Simple
<#while($EncryptionTimes > 0){
$File = Get-Item -Path $outPath -ErrorAction SilentlyContinue
$plainBytes = [System.IO.File]::ReadAllBytes($File.FullName)
$encryptor = $aes.CreateEncryptor()
$encryptedBytes = $encryptor.TransformFinalBlock($plainBytes, 0, $plainBytes.Length)
$encryptedBytes = $aes.IV + $encryptedBytes
[System.IO.File]::WriteAllBytes($outPath, $encryptedBytes)
$EncryptionTimes=$EncryptionTimes--
}#End WHILEENCRYPTIONTIMES #>
$aes.Dispose()
